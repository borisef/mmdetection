...
work_dir = '/home/borisef/projects/mmdetHack/Runs/results_traffic_resnet18_mosaic'
model = dict(
    type='FasterRCNN',
    backbone=dict(
        type='ResNet',
...
train_pipeline_in_mosaic = [

    dict(type='LoadImageFromFile'),
    dict(type='LoadAnnotations', with_bbox=True)
]

train_pipeline_mosaic = [
    dict(type='Mosaic', img_scale=img_scale, pad_val=114.0),
    dict(
        type='Resize',
        # img_scale=[(1333, 640), (1333, 672), (1333, 704), (1333, 736),
...
#----------A---------
train_dataset_multi_one_dataset = dict(
    type='MultiImageMixDataset',
    dataset={**train_data1, 'pipeline':train_pipeline_in_mosaic},
    pipeline=train_pipeline_mosaic
)
#-----------B----------
train_data_concat_mosaic_and_simple = dict(
    type = 'ConcatDataset',
    datasets = [train_dataset_multi_one_dataset, train_data2],

)
#----------C-----------
train_data_concat_for_multi = dict(
    type = 'ConcatDataset',
    datasets = [{**train_data1, 'pipeline':train_pipeline_in_mosaic},
                {**train_data2, 'pipeline':train_pipeline_in_mosaic}],

)
train_data_multi_concat_dataset = dict(
    type='MultiImageMixDataset',
    dataset=train_data_concat_for_multi,
    pipeline=train_pipeline_mosaic
)
#-----------------------------------------
